...
const int AI[nodes+1] = {0, 3, 6, 9, 13, 15, 18, 20, 22};
...
//doubling nodes 3-4 capacity -> double weights in positions 5 and 8 double delta = 1.0;
double delta = 1.0;
...
void derivs(double *y, double *dydt){
  ...
  for (int i=0; i<nodes; i++){
    ...
    dydt[i] = y[i+nodes];
    dydt[i+nodes] = -alpha*y[i+nodes] - Pmax*tanh(delta*y[i]) + P[i] - sum; //Pmax*tanh(delta*y[i]) + P[i] - sum;
